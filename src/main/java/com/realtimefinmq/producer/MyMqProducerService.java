...
public class MyMqProducerService {
    ...
    public boolean publish(String payload) {
        ...
        final Message msg = new Message(id, payload, ts);   // 공용 DTO

        log.debug("[MyMQ-Producer] 메시지 생성 | id={} | ts={} | payload={}", id, ts, payload);
        ...
        try {
            ...
            if (!accepted) {
                ...
                log.warn("[MyMQ-Producer] enqueue 실패/우회(DLQ 가능성) | id={}", id);
                return false;
            } else {
                log.debug("[MyMQ-Producer] 메시지 발행 완료 | id={}", id);
                metrics.incUncommitted(); // 언커밋 +1
                return true;
            }
        } catch (Exception e) {
            // enqueue 중 예외 (WAL 실패 / 디스크 오류 / 장애주입 등)
            log.error("[MyMQ-Producer] 메시지 발행 실패 | id={} | 이유={}", id, e.getMessage(), e);
            return false;
        }
    }
}